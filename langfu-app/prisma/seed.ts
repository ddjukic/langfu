      ...
      for (const wordData of words) {
        const word = await prisma.word.create({
          data: {
            ...
            difficulty: level === 'A1' ? 1 : level === 'A2' ? 2 : level === 'B1' ? 3 : level === 'B2' ? 4 : level === 'C1' ? 5 : 6,
...
function getExampleSentences(word: string, translation: string, language: Language) {
  ...
  if (language === Language.GERMAN) {
    if (word === 'aufstehen') {
      return [
        { sentence: 'Ich stehe jeden Morgen um 7 Uhr auf.', translation: 'I get up every morning at 7 o\'clock.' },
        { sentence: 'Wann stehst du am Wochenende auf?', translation: 'When do you get up on the weekend?' },
      ];
    }
    if (word === 'frühstücken') {
      return [
        { sentence: 'Wir frühstücken zusammen in der Küche.', translation: 'We have breakfast together in the kitchen.' },
        { sentence: 'Er frühstückt nie vor der Arbeit.', translation: 'He never has breakfast before work.' },
      ];
    }
  }
  ...
  // Default examples
  return [
    { sentence: `Example sentence with ${word}.`, translation: `Example sentence with ${translation}.` },
  ];
}
