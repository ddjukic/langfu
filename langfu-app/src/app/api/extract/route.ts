...
function detectLanguage(text: string): Language | null {
  ...
  const germanPatterns =
    /\b(der|die|das|und|ist|ich|ein|eine|haben|werden|nicht|mit|auf|für|aber|nach|bei|über|unter|zwischen)\b/gi;
  // Common Spanish words and patterns
  const spanishPatterns =
    /\b(el|la|los|las|y|es|un|una|que|de|en|por|para|con|pero|como|más|muy|todo|esta)\b/gi;
...
Return a JSON object with this exact structure:
{
  "words": [
    ...
    const estimatedLevel = Object.entries(levelCounts).sort((a, b) => b[1] - a[1])[0]?.[0] || 'B1';
